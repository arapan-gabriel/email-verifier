...
| 021 🚧 | ip-health-zone-coverage | `SelfTest` runs **per zone** — survivors kept, failures dropped and logged, checking disabled only when none pass — so a third zone can be added without risking the two that work. Abusix is configured rather than defaulted: its query name carries a subscription key, which the existing `<reversed-ip>.<zone>` construction already produces, so no query code changed. Prompted by ladder day 4, where `550 5.7.1 ... blocked using mail.abusix.zone` arrived from a receiving server while `/admin/ip-health` said `burned: false` — truthfully, because the default pair was clean and the list that refused us was not among them | 🚧 **Code complete, gate blocked on a human step.** Three unit tests drive the per-zone shapes; the live half needs an Abusix account (free, and the same one that files the delisting) before `blocklist checking enabled` can name three zones |
