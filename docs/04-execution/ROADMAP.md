...
| 020 | turn-on-ip-health-and-suppression | 010 and 011 are ✅ above and **switched off in production**: `ip_health.resolvers` is empty and `suppress.enabled` is false, with two test hashes in Redis. Both were deliberately opt-in and nobody opted in. A local recursive resolver on the node (the zones refuse public ones — measured), the salt on both sides, and a real digest export | startup warns about neither; a deliberately listed address reports **listed**, so the check has been seen to fire; a suppressed digest is refused before any socket opens |
