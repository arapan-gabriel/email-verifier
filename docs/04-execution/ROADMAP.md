...
| 018 | explain-a-verdict | the ladder holds on any `block` movement, and `block` moved — 0 of 102 on day 1, 3 of 100 on day 2. Two of the four are M365 answering `5.4.1` normally, two run their own MX and might be real, and **nothing recorded tells them apart**: neither repository keeps the reply. Log it here (no local part, ever), carry it into `signals` there | a day-2 `block` is explained from the journal, and from its verdict row alone, with no local part in any log line |
