...
| 019 ✅ | ~~envelope-sender-isolation~~ **done 2026-09-11** | the verification envelope moves to `verify@probe.datascoutmail.com`, keeping probing reputation off the domain that will send real mail in Phase C | ✅ `gammait.net` — which answered `554 5.1.8 Sender address rejected` in plan 001 — now gets past `MAIL FROM` and replies `450 4.1.1` about the **recipient** |
...
| 021 ✅ | ~~ip-health-zone-coverage~~ **done 2026-09-14** | `SelfTest` runs **per zone** — survivors kept, failures dropped and logged, checking disabled only when none pass — so a third zone can be added without risking the two that work. Abusix is configured rather than defaulted: its query name carries a subscription key, which the existing `<reversed-ip>.<zone>` construction already produces, so no query code changed. Prompted by ladder day 4, where `550 5.7.1 ... blocked using mail.abusix.zone` arrived from a receiving server while `/admin/ip-health` said `burned: false` — truthfully, because the default pair was clean and the list that refused us was not among them | ✅ live, on the 10:39 UTC deploy of `224b0f9`: three zones named with the Abusix key shown as `<key>`, three `ip_health_listed` series at `0`, `burned: false`; with 32 zeros for a key the Abusix zone is dropped and logged while Spamhaus and SpamCop stay checked, and the good env restores to three |
| 022 | read-the-whole-reply *(active 2026-09-15)* | `readReply` kept only a reply's last line, so Hetzner's seven-line `550` refusing our IP for spam arrived as `550 Ihrem Serveranbieter erfahren.` and was classed **invalid** — invariant 1 — while Data Scout's stop rule had nothing to match. The whole reply is now returned, bounded to 4096 bytes, and blocklist prose (`rbl.`, `spam we are receiving`, `spamaufkommen`, …) classes `policy` | a probe to a Hetzner-hosted recipient returns every line; the journal's `policy` line carries the reason. **Not yet deployed** |
